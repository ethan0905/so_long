...
int	draw_chimney(t_test *test, int random_obj, int x, int y)
{
	if (random_obj == 0)
	{
		draw_on_image(test, &test->all.chimney.bottom_left, x, y
			- 32 - 16 + 8);
		draw_on_image(test, &test->all.chimney.bottom_right, x + 64, y
			- 32 - 16 + 8);
		draw_on_image(test, &test->all.chimney.mid_left, x, y
			- 32 - 16 - 64 + 8);
		draw_on_image(test, &test->all.chimney.mid_right, x + 64, y
			- 32 - 16 - 64 + 8);
		draw_on_image(test, &test->all.chimney.top_left, x, y
			- 32 - 16 - 128 + 8);
		draw_on_image(test, &test->all.chimney.top_right, x + 64, y
			- 32 - 16 - 128 + 8);
...
void	check_which_furnitures(t_test *test)
{
	if (test->param.map[test->iter.i][test->iter.j] == '1' && test->iter.i != 1)
		...
		test->all.random_obj = draw_chimney(test, test->all.random_obj,
				test->iter.x, test->iter.y);
	else if (cond3(test) == 1)
		test->all.ispiano = draw_piano(test, test->all.ispiano,
				test->iter.x, test->iter.y);
...
void	draw_furnitures(t_test *test)
{
	...
	while (test->param.map[++test->iter.i + 1])
	{
		...
		{
			test->iter.x = (64 + (test->param.height - 2 - test->iter.i)
					* 64 + (test->iter.j - 1) * 64);
